...
const dashboardTabs = ['Profile', 'Watchlists', 'History'];
const adminTabs = ['Movies', 'Genres', 'Credits'];
...
function App() {
  ...
  const [loading, setLoading] = useState({ movies: false, detail: false, profile: false, history: false, watchlists: false, page: true });
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      setLoading((prev) => ({ ...prev, history: true, watchlists: true }));
      try {
        ...
        setLoading((prev) => ({ ...prev, history: false, watchlists: false }));
      }
    };
  ...
  const attachCredit = async (event) => {
    ...
    try {
      await genreApi.attachToMovie(Number(creditForm.movie_id), { genre_id: Number(creditForm.person_type) });
      showToast('Genre attached to movie');
    } catch (error) {
        ...
        <div className="space-y-6 py-8">
          ...
          {adminTab === 'Movies' && (
            <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
              <form onSubmit={createMovie} className="glass-panel rounded-[28px] border border-slate-800/80 p-5">
                <h3 className="mb-4 text-xl font-bold text-white">Create movie</h3>
                <div className="space-y-3">
                  <input value={movieForm.title} onChange={(event) => setMovieForm((current) => ({ ...current, title: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Title" required />
                  <textarea value={movieForm.description} onChange={(event) => setMovieForm((current) => ({ ...current, description: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Description" rows="4" required />
                  <div className="grid grid-cols-2 gap-3">
                    <input value={movieForm.release_year} onChange={(event) => setMovieForm((current) => ({ ...current, release_year: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Release year" />
                    <input value={movieForm.duration} onChange={(event) => setMovieForm((current) => ({ ...current, duration: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Duration" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <input value={movieForm.language} onChange={(event) => setMovieForm((current) => ({ ...current, language: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Language" />
                    <input value={movieForm.rating} onChange={(event) => setMovieForm((current) => ({ ...current, rating: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Rating" />
                  </div>
                  <input value={movieForm.poster_url} onChange={(event) => setMovieForm((current) => ({ ...current, poster_url: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Poster URL" />
                  <input value={movieForm.trailer_url} onChange={(event) => setMovieForm((current) => ({ ...current, trailer_url: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Trailer URL" />
                  <button type="submit" className="w-full rounded-full bg-gradient-to-r from-cyan-400 to-indigo-500 px-4 py-3 font-semibold text-slate-950">Create movie</button>
                </div>
              </form>

              <div className="glass-panel rounded-[28px] border border-slate-800/80 p-5">
                <h3 className="mb-4 text-xl font-bold text-white">Recent movies</h3>
                <div className="space-y-3">
                  {movies.slice(0, 6).map((movie) => (
                    <div key={movie.movie_id || movie.id} className="flex items-center justify-between rounded-2xl border border-slate-700 bg-slate-900/40 p-3">
                      <div>
                        <p className="font-semibold text-white">{movie.title}</p>
                        <p className="text-sm text-slate-400">{movie.release_year || 'N/A'} • {movie.language || 'Unknown'} </p>
                      </div>
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-1 text-sm text-amber-300"><Star className="h-3.5 w-3.5 fill-amber-400" />{toDisplayNumber(movie.rating)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {adminTab === 'Credits' && (
            <div className="glass-panel rounded-[28px] border border-slate-800/80 p-5">
              <h3 className="mb-4 text-xl font-bold text-white">Attach cast/crew credit</h3>
              <form onSubmit={attachCredit} className="grid gap-3 md:grid-cols-4">
                <input value={creditForm.name} onChange={(event) => setCreditForm((current) => ({ ...current, name: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Person name" required />
                <select value={creditForm.person_type} onChange={(event) => setCreditForm((current) => ({ ...current, person_type: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white">
                  <option value="actor">Actor</option>
                  <option value="director">Director</option>
                  <option value="writer">Writer</option>
                </select>
                <input value={creditForm.character_name} onChange={(event) => setCreditForm((current) => ({ ...current, character_name: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Character name" />
                <input value={creditForm.movie_id} onChange={(event) => setCreditForm((current) => ({ ...current, movie_id: event.target.value }))} className="w-full rounded-2xl border border-slate-700 bg-slate-900/60 px-3 py-2 text-white" placeholder="Movie ID" required />
                <button type="submit" className="md:col-span-4 w-full rounded-full bg-gradient-to-r from-cyan-400 to-indigo-500 px-4 py-3 font-semibold text-slate-950">Save credit</button>
              </form>
            </div>
          )}
        </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 pb-20 pt-6 sm:px-6 lg:px-8">
        <header className="glass-panel sticky top-4 z-30 flex items-center gap-3 rounded-2xl border border-slate-800/80 px-3 py-3 sm:px-4">
          ...
          <div className="ml-auto hidden items-center gap-3 md:flex">
            <button type="button" onClick={() => { if (token) { setDashboardTab('Watchlists'); window.location.hash = '/'; setRoute('/'); } else { window.location.hash = '/login'; setRoute('/login'); } }} className="rounded-full border border-slate-700/80 bg-slate-900/50 px-3 py-2 text-sm text-slate-200 transition hover:border-cyan-400/80 hover:text-white">Watchlist</button>
            <button type="button" onClick={() => { if (token) { setDashboardTab('History'); window.location.hash = '/'; setRoute('/'); } else { window.location.hash = '/login'; setRoute('/login'); } }} className="rounded-full border border-slate-700/80 bg-slate-900/50 px-3 py-2 text-sm text-slate-200 transition hover:border-cyan-400/80 hover:text-white">History</button>
            <button type="button" onClick={() => { if (token) { setDashboardTab('Profile'); window.location.hash = '/'; setRoute('/'); } else { window.location.hash = '/login'; setRoute('/login'); } }} className="flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-medium text-slate-900 transition hover:bg-white"><UserRound className="h-4 w-4" />{user ? user.name : 'Profile'}</button>
          </div>
